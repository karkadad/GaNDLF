...
def main():
    ...
    train_dataloader, validation_dataloader, parameters = load_data(
        args.train_data_csv, args.valid_data_csv, parameters
    )
    ...
    nncf_config = register_default_init_args(
        nncf_config, train_dataloader, model_eval_fn=eval_fn
    )
    ...
    def validate_fn(compressed_model, epoch=None):
        return validate_network(
            compressed_model, validation_dataloader, None, parameters
        )
    ...
    def train_epoch_fn(compression_ctrl, compressed_model, optimizer, **kwargs):
        return nncf_train_network(
            compressed_model, train_dataloader, optimizer, compression_ctrl, parameters
        )
